...
#Replace content of bracket if you want to use all scaffolds, refer to 5_SHA_CUTandRUN_setup_intersect_directories_generic.sh for details
for i in $(awk '$1 !~ /^NW/' ${GTF} | awk '$3 == "gene"' | cut -f1 | sort | uniq); do
    ...
    awk -F '\t' 'BEGIN {OFS = "\t"} {for (j=$2; j<$3; j++) print $1, j, j+1, $4, $5, $6, $7, $8}' | \
...
#remove the combine directory if you want to keep compressed output only
#rm -rf combine
