...
histones <- c("H3K27me3", "H3K4me3", "H3K9me3", "H4K20me3")
...
for (histone in histones) {
  ...
  #Read data normal
  data_F <- fread(paste("F_Br_", histone, "_info_simplified.tsv", sep = ""), fill = T, drop = c("mod", "sex"))
  data_M <- fread(paste("M_Br_", histone, "_info_simplified.tsv", sep = ""), fill = T, drop = c("mod", "sex"))
  
  #Read data background
  setwd("/path/to/background/plottingdata")
  
  data_F_bg <- fread(paste("F_Br_", histone, "_info_simplified_lambda.tsv", sep = ""), fill = T, drop = c("mod", "sex"))
  data_M_bg <- fread(paste("M_Br_", histone, "_info_simplified_lambda.tsv", sep = ""), fill = T, drop = c("mod", "sex"))
  ...
  mapped_reads_F <- filter(mapped_reads, Histone_Mod == histone, Sex == "F", Tissue == "Br")$Mapped
  mapped_reads_M <- filter(mapped_reads, Histone_Mod == histone, Sex == "M", Tissue == "Br")$Mapped
  ...
  window_ratios %>% ggplot(aes(x = window_center, y = ratio, color = sex_c)) +
    ...
    labs(x = "Position relative to TSS (kb)", y=paste("Ratio (sample/background) of mean peak height of", histone, "in brain")) +
    coord_cartesian(x=c(-10000,10000), y=c(0,1.2))+
    theme_classic() +
  ...
  ggsave(paste("/path/to/R_plots/TSS_", histone, "_backgroundratio_plot_mappedreads_MEU_brain_maledoubled_y_1.2.pdf", sep = ""), width = 7, height = 7, dpi = 800)
}

write.csv(results, "/path/to/MEU_CUTandRUN/R_plots/mappedreads_scaling_backgroundratio_MEU_brain_maledoubled.csv", row.names = FALSE)



#Clean up before next loop
rm(results, data_F, data_M, data_F_bg, data_M_bg, meddata, meddata_bg, meddata_F, meddata_M, meddata_F_bg, meddata_M_bg)
gc()
...
for (histone in histones) {
  ...
  #Read data normal
  data_F <- fread(paste("F_Ki_", histone, "_info_simplified.tsv", sep = ""), fill = T, drop = c("mod", "sex"))
  data_M <- fread(paste("M_Ki_", histone, "_info_simplified.tsv", sep = ""), fill = T, drop = c("mod", "sex"))
  
  #Read data background
  setwd("/path/to/background/plottingdata")
  
  data_F_bg <- fread(paste("F_Ki_", histone, "_info_simplified_lambda.tsv", sep = ""), fill = T, drop = c("mod", "sex"))
  data_M_bg <- fread(paste("M_Ki_", histone, "_info_simplified_lambda.tsv", sep = ""), fill = T, drop = c("mod", "sex"))
  ...
  #Get the number of mapped reads for the current histone and sex
  mapped_reads_F <- filter(mapped_reads, Histone_Mod == histone, Sex == "F", Tissue == "Ki")$Mapped
  mapped_reads_M <- filter(mapped_reads, Histone_Mod == histone, Sex == "M", Tissue == "Ki")$Mapped
  ...
  window_ratios %>% ggplot(aes(x = window_center, y = ratio, color = sex_c)) +
    geom_smooth(aes(fill=sex_c), se= T, method = "loess", span=0.35) +
    labs(x = "Position relative to TSS (kb)", y=paste("Ratio (sample/background) of mean peak height of", histone, "in kidney")) +
    coord_cartesian(x=c(-10000,10000), y=c(0,1.2))+
    theme_classic() +
    scale_color_manual(values = c("plum2", "mediumorchid4", "darkorange", "orangered"), name = "", labels = c("Female autosomes", "Female Xs", "Male autosomes", "Male Xs"))+
    scale_fill_manual(values = c("plum2", "mediumorchid4", "darkorange", "orangered"), name = "", labels = c("Female autosomes", "Female Xs", "Male autosomes", "Male Xs"))+
  ...
  ggsave(paste("/path/to/R_plots/TSS_", histone, "_backgroundratio_plot_mappedreads_MEU_kidney_maledoubled_y_1.2.pdf", sep = ""), width = 7, height = 7, dpi = 800)
}

write.csv(results, "/path/to/MEU_CUTandRUN/R_plots/mappedreads_scaling_backgroundratio_MEU_kidney_maledoubled.csv", row.names = FALSE)
