...
##the first awk command filters out the unplaced scaffolds (gets rid of NW and keeps the NC). If you want all scaffolds instead, replace content inside bracket below with the alternate commented line.
##(awk '$3 == "gene"' ${GTF} | cut -f1 | sort | uniq)
##NOTE: need to also replace content of bracket in 6_MEU_CUTandRUN_bedtools_intersect_pileup_gtf_generic.sh if you want to use all scaffolds
...
cd ${topdir}/pileup
for i in $(ls *.bdg); do
    qsub -o ${topdir}/log -v topdir=${topdir},workingdir=${topdir}/workingdir/${i/.bdg},BDG=${topdir}/pileup/${i},GTF=${GTF} /path/to/6_MEU_CUTandRUN_bedtools_intersect_pileup_gtf_generic.sh
done
